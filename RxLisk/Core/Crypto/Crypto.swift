...
public struct Crypto {
    ...
    /// Generate public key from a given secret
    public static func publicKey(fromSecret secret: String) throws -> String {
        let keyPair = try self.keyPair(fromSecret: secret)
        return keyPair.publicKeyString
    }
    
    /// Generate private key from a given secret
    public static func privateKey(fromSecret secret: String) throws -> String {
        let keyPair = try self.keyPair(fromSecret: secret)
    ...
    /// Extract Lisk address from a secret
    public static func address(fromSecret secret: String) throws -> String {
        let publicKey = try self.publicKey(fromSecret: secret)
        return self.address(fromPublicKey: publicKey)
    }
    ...
    public static func timeIntervalSinceGenesis(offset: TimeInterval = 0) -> UInt32 {
        ...
        return UInt32(now - LiskConstants.Time.epochSeconds)
    }
